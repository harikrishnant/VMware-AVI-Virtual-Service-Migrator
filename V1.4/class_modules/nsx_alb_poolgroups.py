...
class NsxAlbPoolGroup:
    ...
    def get_poolgroup(self):
        ...
        while new_results:
            ...
            page += 1
        if (len(self.dict_poolgroup_url_name) == 0) and (response == False): #Handle a scenario where there are no pool groups in VMware AVI tenant
            self.print_func("\n" + '\033[91m' + f"ERROR: List VMware AVI Pool Groups Unsuccessful ({response.status_code})" + '\033[0m' + "\n")
            self.print_func(tabulate(list(map(list, response.json().items())), headers=["Error", "Details"], showindex=True, tablefmt="fancy_grid"))
            sys.exit()
        
        #Block to scan for poolgroups with name duplication
        self.df_duplicate_poolgroups = pandas.DataFrame(
            {
                "obj_type" : [],
                "obj_name" : [],
                "obj_ref" : [],
        ...
        dict_dup_counter = Counter(self.dict_poolgroup_url_name.values())
        list_dup_names = [each for each in dict_dup_counter if dict_dup_counter[each] > 1]
        if list_dup_names:
            for each_poolgroup_name in list_dup_names:
                for poolgroup in self._list_poolgroups:
                    if each_poolgroup_name == poolgroup["name"]:
                        df_dup_each_item = pandas.DataFrame(
                            {
                                "obj_type" : ["poolgroup"],
                                "obj_name" : [poolgroup["name"]],
                                "obj_ref" : [poolgroup["url"]],
                                "tenant" : [self._headers["X-Avi-Tenant"]] 
                            }
                        )
                        self.df_duplicate_poolgroups = pandas.concat([self.df_duplicate_poolgroups, df_dup_each_item], ignore_index=True)
    ...
    def create_poolgroup(self, body):
        ''' Class Method to create VMware AVI Pool Groups in the Tenant on the selected Cloud Account'''
        response = requests.post(self._url, json=body, headers=self._headers, verify=False )
        if response:
            self.print_func("\n" + '\033[92m' + f"Pool Group '{response.json()['name']}' successfully created ({response.status_code})" + '\033[0m')
        else:
            self.print_func("\n" + '\033[91m' +f"ERROR: Pool Group'{body['name']}' creation Failed ({response.status_code})" + '\033[0m')
            self.print_func(tabulate(list(map(list, response.json().items())), headers=["Error", "Details"], showindex=True, tablefmt="fancy_grid"))
    ...
    def migrate_poolgroup(self, dict_poolgroup2migrate_url_name, dict_originalpoolurl_migratedpoolurl, target_cloud_url, prefix_tag, tracker_csv):   
        '''Class Method to migrate pool groups from one VMware AVI cloud to another in the same Tenant''' 
    ...
    def remove_poolgroup_prefix(self, obj_tracker, headers):
        ''' Class Method to remove the prefixes of VMware AVI poolgroups '''
        ...
        for index, row in df_obj_track_csv.iterrows():
            if row["obj_type"] == "poolgroup":
                if row["url"] in self.dict_poolgroup_url_name.keys():
                    for poolgroup in self._list_poolgroups:
                        if row["url"] == poolgroup["url"]:
                            if poolgroup["name"][:len(self._run_id)] == self._run_id:
                                ...
                                if response:
                                    print("\n" + '\033[92m' + f"Poolgroup Prefix for {row['obj_name']} removed successfully ({response.status_code}). New Object name is '{response.json()['name']}'" + '\033[0m')
                                    ...
                                    print("\n" + '\033[93m' +f"WARNING: Poolgroup Prefix removal failed for {row['obj_name']} - ({response.status_code})" + '\033[0m')
                                    dict_df_obj_remove_prefix_status = {
                                        "obj_type" : ["poolgroup"],
                                        "obj_name_old" : [row["obj_name"]],
                                ...
                                print("\n" + '\033[93m' +f"WARNING: Prefix tag missing in {poolgroup['name']}, hence not renamed" + '\033[0m')
                                dict_df_obj_remove_prefix_status = {
                                        "obj_type" : ["poolgroup"],
                                        "obj_name_old" : [row["obj_name"]],
                    ...
                    print("\n" + '\033[93m' + f"WARNING: poolgroup object '{row['obj_name']}' not found, hence skipped" + '\033[0m')
                    dict_df_obj_remove_prefix_status = {
                            "obj_type" : ["poolgroup"],
                            "obj_name_old" : [row["obj_name"]],
                            "obj_name_new" : ["NONE"],
                            "PREFIX_REMOVAL_STATUS" : ["FAILURE"],
                            "Error" : [f"poolgroup object '{row['obj_name']}' not found"]
                        }  
                    df_obj_remove_prefix_status = pandas.DataFrame(dict_df_obj_remove_prefix_status)
                    df_obj_remove_prefix_status.to_csv(obj_tracker + "/obj_prefix_removal_status_" + self._run_id + ".csv", index=False, mode='a', header=False)
