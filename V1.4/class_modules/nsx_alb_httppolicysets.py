...
class NsxAlbHttpPolicySet:
    ...
    def get_httppolicyset(self):
        ...
        if (len(self.dict_httppolicyset_url_name) == 0) and (response == False): #Handle a scenario where there are no HTTP policy sets defined
            self.print_func("\n" + '\033[91m' + f"ERROR: Unable to scan HTTP Policy Sets ({response.status_code})" +'\033[0m' + "\n")
    ...
    def create_httppolicyset(self, body):
        ...
        if response:
            self.print_func("\n" + '\033[92m' + f"HTTP Policy Set '{response.json()['name']}' successfully created ({response.status_code})" + '\033[0m')
        else:
            self.print_func("\n" + '\033[91m' + f"ERROR: HTTP Policy Set '{body['name']}' creation failed ({response.status_code})" + '\033[0m' + "\n")
            self.print_func(tabulate(list(map(list, response.json().items())), headers=["Error", "Details"], showindex=True, tablefmt="fancy_grid"))
    ...
    def remove_httppolicyset_prefix(self, obj_tracker, headers):
        ...
        for index, row in df_obj_track_csv.iterrows():
            if row["obj_type"] == "httppolicyset":
                if row["url"] in self.dict_httppolicyset_url_name.keys():
                    for httppolicyset in self._list_httppolicysets:
                        if row["url"] == httppolicyset["url"]:
                            if httppolicyset["name"][:len(self._run_id)] == self._run_id:
                                ...
                                if response:
                                    print("\n" + '\033[92m' + f"httppolicyset Prefix for {row['obj_name']} removed successfully ({response.status_code}). New Object name is '{response.json()['name']}'" + '\033[0m' + "\n")
                                    ...
                                    print("\n" + '\033[93m' + f"WARNING: httppolicyset Prefix removal failed for {row['obj_name']} - ({response.status_code})" + '\033[0m')
                                    dict_df_obj_remove_prefix_status = {
                                        "obj_type" : ["httppolicyset"],
                                        "obj_name_old" : [row["obj_name"]],
                                ...
                                print("\n" + '\033[93m' + f"WARNING: Prefix tag missing in {httppolicyset['name']}, hence not renamed" + '\033[0m' + "\n")
                                dict_df_obj_remove_prefix_status = {
                                        "obj_type" : ["httppolicyset"],
                                        "obj_name_old" : [row["obj_name"]],
                    ...
                    print("\n" + '\033[93m' + f"WARNING: httppolicyset object '{row['obj_name']}' not found, hence skipped" + '\033[0m')
                    dict_df_obj_remove_prefix_status = {
                            "obj_type" : ["httppolicyset"],
                            "obj_name_old" : [row["obj_name"]],
                            "obj_name_new" : ["NONE"],
                            "PREFIX_REMOVAL_STATUS" : ["FAILURE"],
                            "Error" : [f"httppolicyset object '{row['obj_name']}' not found"]
                        }  
                    df_obj_remove_prefix_status = pandas.DataFrame(dict_df_obj_remove_prefix_status)
                    df_obj_remove_prefix_status.to_csv(obj_tracker + "/obj_prefix_removal_status_" + self._run_id + ".csv", index=False, mode='a', header=False)
