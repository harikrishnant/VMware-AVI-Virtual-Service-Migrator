...
class NsxAlbSeGroup:
    ...
    def get_segroup(self): 
        ...
        self.dict_segroup_all_url_name = {}
        new_results = True
        page = 1
        while new_results:
            ...
            for segroup in new_results:
                if segroup != []:
                    self.dict_segroup_all_url_name[segroup["url"]] = segroup["name"]
        ...
        if self._migration_mode == "migrate":
            if len(self._dict_segroup_url_name) != 0:
                ...
                self.print_func("\n" + '\033[91m' + f"ERROR: List Service Engine Groups Unsuccessful ({response.status_code})" + '\033[0m' + "\n")
