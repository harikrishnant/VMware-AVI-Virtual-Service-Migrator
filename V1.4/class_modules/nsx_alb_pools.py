...
class NsxAlbPool:
    ...
    def get_pool(self):
        ...
        while new_results: 
            ...
            page += 1
        if (len(self.dict_pool_url_name) == 0) and (response == False): #Handle a scenario where there are no pools in VMware AVI tenant
            self.print_func("\n" + '\033[91m' + f"ERROR: List VMware AVI Pools Unsuccessful ({response.status_code})" + '\033[0m' + "\n")
        ...
        #Block to scan for pools with name duplication
        self.df_duplicate_pools = pandas.DataFrame(
            {
                "obj_type" : [],
                "obj_name" : [],
                "obj_ref" : [],
        ...
        dict_dup_counter = Counter(self.dict_pool_url_name.values())
        list_dup_names = [each for each in dict_dup_counter if dict_dup_counter[each] > 1]
        if list_dup_names:
            for each_pool_name in list_dup_names:
                for pool in self._list_pools:
                    if each_pool_name == pool["name"]:
                        ...
                        self.df_duplicate_pools = pandas.concat([self.df_duplicate_pools, df_dup_each_item], ignore_index=True)
    ...
    def create_pool(self, body):
        ''' Class Method to create VMware AVI pools in the Tenant on the selected Cloud Account'''
        response = requests.post(self._url, json=body, headers=self._headers, verify=False )
        if response:
            self.print_func("\n" + '\033[92m' + f"Pool '{response.json()['name']}' successfully created ({response.status_code})" + '\033[0m')
        else:
            self.print_func("\n" + '\033[91m' + f"ERROR: Pool '{body['name']}' creation Failed ({response.status_code})" + '\033[0m')
            self.print_func(tabulate(list(map(list, response.json().items())), headers=["Error", "Details"], showindex=True, tablefmt="fancy_grid"))
    ...
    def migrate_pool(self, dict_pool2migrate_url_name, target_cloud_url, target_vrfcontext_url, target_vrfcontext_tier1path, prefix_tag, tracker_csv):
        '''Class Method to migrate pools from one VMware AVI cloud to another in the same Tenant'''
    ...
    def remove_pool_prefix(self, obj_tracker, headers):
        ''' Class Method to remove the prefixes of VMware AVI pools '''
        self.get_pool() #get_pool methos is a pre-requisite for calling migrate_pool method
        ...
        for index, row in df_obj_track_csv.iterrows():
            if row["obj_type"] == "pool":
                if row["url"] in self.dict_pool_url_name.keys():
                    for pool in self._list_pools:
                        if row["url"] == pool["url"]:
                            if pool["name"][:len(self._run_id)] == self._run_id:
                                ...
                                if response:
                                    print("\n" + '\033[92m' + f"Pool Prefix for {row['obj_name']} removed successfully ({response.status_code}). New Object name is '{response.json()['name']}'" + '\033[0m')
                                    ...
                                    print("\n" + '\033[93m' + f"WARNING: Pool Prefix removal failed for {row['obj_name']} - ({response.status_code})" + '\033[0m')
                                    dict_df_obj_remove_prefix_status = {
                                        "obj_type" : ["pool"],
                                        "obj_name_old" : [row["obj_name"]],
                                ...
                                print("\n" + '\033[93m' + f"WARNING: Prefix tag missing in {pool['name']}, hence not renamed" + '\033[0m')
                                dict_df_obj_remove_prefix_status = {
                                        "obj_type" : ["pool"],
                                        "obj_name_old" : [row["obj_name"]],
                    ...
                    print("\n" + '\033[93m' + f"WARNING: pool object '{row['obj_name']}' not found, hence skipped" + '\033[0m')
                    dict_df_obj_remove_prefix_status = {
                            "obj_type" : ["pool"],
                            "obj_name_old" : [row["obj_name"]],
                            "obj_name_new" : ["NONE"],
                            "PREFIX_REMOVAL_STATUS" : ["FAILURE"],
                            "Error" : [f"pool object '{row['obj_name']}' not found"]
                        }  
                    df_obj_remove_prefix_status = pandas.DataFrame(dict_df_obj_remove_prefix_status)
                    df_obj_remove_prefix_status.to_csv(obj_tracker + "/obj_prefix_removal_status_" + self._run_id + ".csv", index=False, mode='a', header=False)