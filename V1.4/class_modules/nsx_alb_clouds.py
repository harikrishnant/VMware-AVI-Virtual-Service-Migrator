...
class NsxAlbCloud:
    def __init__(self, url, headers, migration_mode, run_id):
    ...
    def get_cloud(self): 
        ...
        if self._migration_mode == "migrate":
            if len(self._list_clouds) != 0:
                self.print_func(f"\nFound {len(self._list_clouds)} VMware AVI Cloud Accounts under the Tenant - {self._headers['X-Avi-Tenant']}")
                self.print_func(f"\nCloud details below:\n")
                ...
                self.print_func("\n" + '\033[91m' + f"ERROR: List VMware AVI Cloud Accounts Unsuccessful ({response.status_code})" + '\033[0m' + "\n")
