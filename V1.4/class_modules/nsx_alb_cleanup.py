...
class NsxAlbCleanup:
    ...
    def delete_object(self, url, obj_type, obj_name):
        ...
        if response:
            print("\n" + '\033[92m' + f"{obj_type.title()} object [{obj_name}] deleted successfully - ({response.status_code})" + '\033[0m')
            ...
            print("\n" + '\033[91m' + f"{obj_type.title()} object [{obj_name}] deletion failed - ({response.status_code})" + '\033[0m')
            dict_df_obj_deletion_status = {
                "obj_type" : [obj_type],
                "obj_name" : [obj_name],
                "url" : [url],
    ...
    def initiate_cleanup(self):
        ...
        for cust_attr in ["child", "parent", "normal"]: #To make sure VS'es are deleted in this order - child, parent and normal
            for index, row in df_track_csv.iterrows():
                if row["custom_attr"] == cust_attr and row["obj_type"] == "virtualservice":
                   self.delete_object(row["url"], row["obj_type"], row["obj_name"])

        for each_obj in ["vsvip", "httppolicyset", "poolgroup", "pool"]:
            for index, row in df_track_csv.iterrows():
