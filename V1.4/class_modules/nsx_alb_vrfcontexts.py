...
class NsxAlbVrfContext:
    def __init__(self, url, headers, migration_mode, run_id, target_cloud_url="", target_cloud_name=""):
    ...
    def get_vrfcontext(self): 
        ...
        self.list_vrfcontexts = [] #VRF Contexts of target cloud connector
        self._dict_vrfcontext_url_name = {} #VRF Contexts of target cloud connector
        self.list_vrfcontexts_all = [] #VRF Contexts of all cloud connectors
        self.dict_vrfcontext_all_url_name = {} #VRF Contexts of all cloud connectors
        new_results = True
        page = 1
        while new_results:
            ...
            for vrfcontext in new_results:
                if vrfcontext != []:
                    self.list_vrfcontexts_all.append(vrfcontext)
                    self.dict_vrfcontext_all_url_name[vrfcontext["url"]] = vrfcontext["name"]
        ...
        if self._migration_mode == "migrate":
            if len(self._dict_vrfcontext_url_name) != 0:
                ...
                self.print_func("\n" + '\033[91m' + f"ERROR: List VRF Contexts Unsuccessful ({response.status_code})" + '\033[0m')
    ...
    def set_vrfcontext(self, target_vrfcontext_name):
        ...
        if self.target_vrfcontext_tier1path != "":
            self.print_func("\n" + '\033[95m' + f"INFO: NSX-T Tier1 Gateway associated with selected VRF Context is '{self.target_vrfcontext_tier1path}'" + '\033[0m' + "\n")