...
class NsxAlbDnsProfile:
    ...
    def scan_dnsprofile(self, target_cloud_dnsprofile_url, selected_dns_domain=[]):
        ...
        if self._domainmapping_dnsprofile:
            ...
            self.print_func("\n" + '\033[91m' + f"ERROR: DNS Profile in Cloud Connector doesn't have DNS domains associated. Tool will now Exit" + '\033[0m' + "\n") 
        ...
        if list_incorrect_selected_domain:
            self.print_func("\n" + '\033[91m' + f"ERROR: {len(list_incorrect_selected_domain)} DNS domains supplied as arguments are not available in Cloud connector, details below: Tool will now exit.\n")
