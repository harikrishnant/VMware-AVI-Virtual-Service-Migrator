...
class NsxAlbIpamProfile:
    ...
    def scan_ipamprofile(self, target_cloud_ipamprofile_url, target_ipam_network, target_ipam_subnet, list_vrfcontexts, target_vrf_name):
        ...
        if target_cloud_ipamprofile_url:
            for each_ipamprofile in self._list_ipamprofile:
                if each_ipamprofile.get("url") == target_cloud_ipamprofile_url:
                    ...
                    if list_usable_networks:
                        ...
                        self.print_func("\n" + '\033[91m' + "ERROR: IPAM profile in Target Cloud account doesn't have valid networks associated. Tool will now Exit" + '\033[0m' + "\n")
            ...
            self.print_func("\n" + '\033[91m' + "ERROR: Target Cloud account doesn't have IPAM profile attached. Tool will now Exit" + '\033[0m' + "\n")
        ...
        if target_ipam_network not in self.dict_network_subnet.keys():
            self.print_func("\n" + '\033[91m' + f"ERROR: Selected IPAM Network '{target_ipam_network}' not found in Cloud Connector" + '\033[0m' + "\n")
        ...
        if target_ipam_subnet not in self.dict_network_subnet.get(target_ipam_network, "")[0]:
            self.print_func("\n" + '\033[91m' + f"ERROR: Selected IPAM Subnet '{target_ipam_subnet}' not found in IPAM Network '{target_ipam_network}'" + '\033[0m' + "\n")
        ...
        if target_vrf_name != self.dict_network_subnet.get(target_ipam_network, "")[1]:
            self.print_func("\n" + '\033[91m' +f"ERROR: IPAM Network '{target_ipam_network}' doesn't belong to the supplied target VRF Context '{target_vrf_name}'" + '\033[0m' + "\n")
            print(tabulate([["VRF Mismatch", f"ERROR: IPAM Network '{target_ipam_network}' doesn't belong to the supplied target VRF Context '{target_vrf_name}'"]], ["Error", "Description"], showindex=True, tablefmt="fancy_grid"))
            sys.exit()
